    ...
    for i in range(len(box_xywh)):
        for j in range(len(box_xywh[i])):
            ...
            if max_val >= score_threshold:
                ...
                box_xy.append(class_boxes[j][:2])
                box_yx.append(box_xy[j][::-1])
                
                box_wh.append(class_boxes[j][2:])
                box_hw.append(box_wh[j][::-1])
...
'''
def Combined_Non_Max_Suppression_NonTF(boxes,pred_conf,num_classes=3,iou_threshold=0.5,use_tf_nms=True,img_size=448):
    if not use_tf_nms:
        labels = []
